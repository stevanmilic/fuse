...
object ExpressionsSpec extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("parse method application with multiple arguments") {
      parse("l.fold_left(0)(sum)") ==> FMethodApp(
        FProj(
          FVar("l"),
          Seq(FVar("fold_left"))
        ),
    ...
    test("parse method application with lambda argument with inline block") {
      parse("l.map(a => {\n a + 1\n })") ==> FMethodApp(
        FProj(
          FVar("l"),
          Seq(FVar("map"))
    ...
    test("parse lambda expression with single param and no parenthesis") {
      parse("a => a + 1") ==> FAbs(
        Seq(FBinding(FIdentifier("a"))),
        None,
        Seq(
