...
object Shifting {
  ...
  def typeShiftAbove(d: Int, c: Int, ty: Type): Type =
    typeMap(
      (info, c, k, n) => if (k >= c) TypeVar(info, k + d, n + d) else TypeVar(info, k, n + d),
      c,
      ty
    )
  ...
  def typeMap(onVar: ShiftVarFunc, c: Int, t: Type): Type = {
    def iter(c: Int, tyT: Type): Type = tyT match {
      ...
      case TypeRecord(info, fieldTypes) =>
        TypeRecord(info, fieldTypes.map { case (l, tyTi) => (l, iter(c, tyTi)) })
      case TypeVariant(info, fieldTypes) =>
        TypeVariant(info, fieldTypes.map { case (l, tyTi) => (l, iter(c, tyTi)) })
      case TypeArrow(info, tyT1, tyT2) => TypeArrow(info, iter(c, tyT1), iter(c, tyT2))
      ...
      case TypeApp(info, tyT1, tyT2)   => TypeApp(info, iter(c, tyT1), iter(c, tyT2))
